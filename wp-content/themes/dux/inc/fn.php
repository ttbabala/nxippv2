...
function _the_logo() {
	...
	echo '<' . $tag . ' class="logo"><a href="' . get_bloginfo('url') . '" title="' . get_bloginfo('name') . _get_delimiter() . get_bloginfo('description') . '"><img src="'._hui('logo_src').'" width="215px" height="78px">' . get_bloginfo('name') . '</a></' . $tag . '>';
...
function getdecatLink($catid){        //获取指定分类链接
   /* if(is_array($catid) == False){
      return get_category_link($catid);     
    } */
    if(substr_count($catid,',') == 0){
        return get_category_link($catid);
    }else{
        return get_category_link($catid);
    }
